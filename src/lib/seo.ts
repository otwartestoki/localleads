...
export function pageMetadata({title,description,path='/',image=ogImage,noIndex=false}:{title:string;description:string;path?:string;image?:string;noIndex?:boolean;}): Metadata {
 ...
 return {title:fullTitle,description,alternates:{canonical:absoluteUrl(path)},openGraph:{type:'website',locale:'pl_PL',siteName:site.brand?.name||'LocalLeads',title:fullTitle,description,url:absoluteUrl(path),images:[{url:absoluteUrl(image),width:1200,height:630,alt:'Darmowa baza firm online - LocalLeads'}]},twitter:{card:'summary_large_image',title:fullTitle,description,images:[absoluteUrl(image)]},robots:noIndex?{index:false,follow:false}:{index:true,follow:true,googleBot:{index:true,follow:true,'max-video-preview':-1,'max-image-preview':'large','max-snippet':-1}}};
}
...
export function serviceJsonLd(){return {'@context':'https://schema.org','@type':'Service',name:'Darmowa baza firm online',provider:{'@type':'Organization',name:site.brand?.name||'LocalLeads',url:siteUrl},areaServed:'Polska',serviceType:'Baza firm online',description:site.seo?.description||site.description,offers:{'@type':'Offer',priceCurrency:'PLN',price:'0',url:siteUrl}}}
...
export function productJsonLd({name,description,path}:{name:string;description:string;path:string;}){return {'@context':'https://schema.org','@type':'Product',name,description,url:absoluteUrl(path),brand:{'@type':'Brand',name:site.brand?.name||'LocalLeads'},category:'Baza firm online',offers:{'@type':'Offer',priceCurrency:'PLN',price:'0',availability:'https://schema.org/InStock',url:absoluteUrl(path)}}}
